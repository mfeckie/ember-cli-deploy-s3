...
module.exports = CoreObject.extend({
  ...
  _putObject: function(filePath, options, filePaths) {
    ...
    var brotliFilePaths      = options.brotliFilePaths || [];
    ...
    var isBrotli    = brotliFilePaths.indexOf(filePath) !== -1;
    ...
    if (isBrotli && path.extname(basePath) === '.br') {
      var brotliUncompressed = path.basename(basePath, '.br');
      if (filePaths && filePaths.indexOf(brotliUncompressed) !== -1) {
        contentType = mime.lookup(brotliUncompressed);
        encoding = mime.charsets.lookup(contentType);
      }
    }
